$(document).ready(function () {
    ...
    var editPlaylistId = urlParams.split("?playlistId=")[1];
    var scrollAmount = $(document).height() - $('.section.content').height();
    var playlistName = '';
    var playlist;
    var fileList = [];
    var currentFolderToUpload = "content";
    $('.content-header').children('h1').text('Playlist Editing');


    //ajax-call to get playlist by id
    $.ajax({
        url: "rest/playlist/getPlaylistById/" + editPlaylistId,
        type: "GET",
        dataType: "json",
        ...
        success: function (result) {
            ...
            $('.playlist-name').children('.floatlabel').addClass('open');
            $('.playlist-name').find('#playlistName').val(result.name);
            checkFolatLabel();
            for (var i in result.media) {
                var fileNameEdit;
                var fileExt;
                var fileDropdownState = 'hide';
                var fileResetState = 'hide';
                var fileSetTimeState = '';
                fileNameEdit = result.media[i].filename;
                var folder = fileNameEdit.split("/")[0];
                var file = fileNameEdit.split("/")[1];
                if (fileNameEdit.split("/")[1]) {
                    fileNameEdit = fileNameEdit.split("/")[1];
                    file = fileNameEdit;
                } else {
                    folder = "content";
                };
                var fileExt = fileNameEdit.substring(fileNameEdit.lastIndexOf(".")).split(".")[1];
                var totalSec = result.media[i].duration;
                var hours = parseInt(totalSec / 3600) % 24;
                var minutes = parseInt(totalSec / 60) % 60;
                var seconds = totalSec % 60;

                if (seconds == '' && minutes == '') {
                    fileDropdownState = 'hide';
                    fileResetState = 'hide';
                    fileSetTimeState = '';
                } else {
                    fileDropdownState = '';
                    fileResetState = '';
                    fileSetTimeState = 'hide';
                };

                //draw duration dropdown
                var durationOutputBlock = '<div class="dropdown dropdown-duration minutes ' + fileDropdownState + '">' +
                    '<button id="dLabel" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">' +
                    '<span class="count">' + minutes + '</span> minutes' +
                    ...
                    '</div>' +
                    '</div>' +
                    '<div class="dropdown dropdown-duration seconds ' + fileDropdownState + '">' +
                    '<button id="dLabel" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">' +
                    '<span class="count">' + seconds + '</span> seconds' +
                    '<span class="caret"></span>' +
                    '</button>' +
                    '<div class="dropdown-menu" aria-labelledby="dLabel">' +
                    '<input type="text form-control">' +
                    '</div>' +
                    '</div>' +
                    '<button class="reset reset-duration-action ' + fileResetState + '"><span>Reset</span></button>' +
                    '<button class="set-duration set-duration-action ' + fileSetTimeState + '"><span>Set Duration</span></button>';

                $('.selected-files').find('tbody').append('<tr data-file="' + fileNameEdit + '"data-folder="' + folder +
                    '"data-hours="' + hours + '" data-minutes="' + minutes + '" data-seconds="' + seconds + '" ' +
                    'id="' + result.media[i].mediaId + '">' +
                    '<td class="video-title"><div class="btn-draggable"></div>' + fileNameEdit + '</td>' +
                    '<td>' +
                    durationOutputBlock
                    + '<div class="actions">' +
                    ...
                    '<span>Duplicate</span>' +
                    '</button>' +
                    '<button class="btn btn-danger deleteFileAction"><span class="icon icon-uEA01-trash"></span></button></div></td>'+
                    '</tr>');
            };
    ...
    if (isElementExist('#filesDialog')) {
        $('#filesDialog').on('shown.bs.modal', function () {
            $(this).find('.modal-body').css('height', $(window).height() - ($(this).find('.modal-header').outerHeight() + $(this).find('.modal-footer').outerHeight()));1
        ...
        $.getJSON(playlistContentData, function (data, i) {
            ...
            $.each(data, function (title, data) {
                ...
                $('#foldersNav').children('ul').append('<li class="folder"><a href="#' + title + '"><span>' + title + '</span></a>' +
                    '<button class="upload-button" type="button" id="upload-button" data-toggle="modal" data-target="#fileuploadModal" title="Upload File">' +
    ...
    //delete media-file from playlist
    ...
    //end link manipulation


    //continue set media to playlist
    $("#continueButton").click(function () {
        ...
        $.each($('.files-view').children('section').children('div.file').children('div.item.selected'), function (i, file) {
            console.log("selected: " + $(this).attr('data-folder') + "/" + $(this).attr('data-file'));
            fileList.push({filename: $(this).attr('data-folder') + "/" + $(this).attr('data-file')});
            $(tableContents).children('tbody').append('<tr data-minutes="0" data-seconds="0" ' +
                'data-folder="' + $(this).attr('data-folder') + '" data-file="' + $(this).attr('data-file') + '" id="' + file.mediaId + '">' +
                '<td class="video-title">' +
                '<div class="btn-draggable"></div>' + file.innerText +
                '</td>' +
                '<td>' +
                '<div class="dropdown dropdown-duration hide minutes">' +
                '<button id="dLabel" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">' +
    ...
    //end continue set media to playlist
    ...
    $("#savePlaylist").click(function () {
        ...
        playlist = {playlistId: editPlaylistId, name: playlistName, media: fileList};
        console.log("Updated playlist: " + JSON.stringify(playlist));
        $.ajax({
            url: "rest/playlist/updatePlaylist",
            type: "POST",
            dataType: "json",
